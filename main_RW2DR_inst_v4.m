...
%% set parameters
% general parameters
nses = 4; % we have 3 sessions in total
npos = 40; % number of position bins 
...
Tpara.RWStep = 4; % Gaussian smooth kernel of random walker, large value leads to more diffusive random walker
...
for iplot = 1:runtest
    ...
    for ises = 1:nses
        ...
        % policy based on experience
        [Act,net,confi,rwdacc,allact,allrwd,allestrwd] = DynShRT_inst_v4(postlen,rwdtlen,npos,seqlen,net,rp,confi,rwdacc,allact,allrwd,allestrwd,Tpara,ax);
        if ises == 1
            if strcmp(rp,'TLeftRight')
                hd(1) = plot(nan,nan,'-k');
