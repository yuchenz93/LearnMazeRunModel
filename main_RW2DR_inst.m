...
% track ends
% in this version, animal would update reward policy not at the end of
% session but as long as they accumulate enough new experience
clear;clc;close all
dbstop if error
%% set parameters
% general parameters
...
Tpara.learnstep = 50; % retrain reward policy every 50 steps
...
OutPath = '/media/yuchen/data14/CAResults/DirectionRun/L2R/';
mkdir(OutPath)
Tpara.test = 0;
...
for iplot = 1:runtest
    ...
    for ises = 1:nses
        ...
        % policy based on experience
        [Act,net,confi,rwdacc,allact,allrwd] = DynShRT_inst(tlen,npos,seqlen,net,rp,confi,rwdacc,allact,allrwd,Tpara,ax);
        if ises == 1
            hd(1) = plot(nan,nan,'-k');
            hd(2) = plot(nan,nan,'or');
            legend(hd,{'Trajectory','Reward collected'})
        end
    end
