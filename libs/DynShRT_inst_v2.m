function [Shall,net,confi,rwdacc,allact,allrwd,allestrwd] = DynShRT_inst_v2(tlen,npos,seqlen,net,rp,confi,rwdacc,allact,allrwd,allestrwd,Tpara,ax)
...
for istep = 2:seqlen
    ...
    allestrwd = cat(2,allestrwd,rnow);
    ...
    % reward
    [~,pctloss] = VCNTy_TrackRewardLoss(allact,allestrwd,allrwd,npos);
...
%     confi = min(1,confi);
    ...
    tgap = tgap + 1;
    if pctloss >= Tpara.RTLossThreshold && rwdacc > 0 && tgap >= tgapcri
...
%         [net,~] = train(net,learnact,learnrwd);
    ...
    if nargin >= 12
       ...
       yyaxis right
       plot(ax,[istep-1,istep],[confiori,confi],'-','color',[0.56,0.56,0.3]) 
       drawnow
    end
end
