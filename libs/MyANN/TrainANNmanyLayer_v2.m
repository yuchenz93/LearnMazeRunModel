function [net,pctloss,ax1,ax2] = TrainANNmanyLayer_v2(net,x,t,lf,Tpara)
...
for it = 1:itlim
    ...
    for ilayer = 1:nlayer
        if it == 1
            mdW{ilayer} = dW{ilayer}/trainsize;
            mdb{ilayer} = db{ilayer}/trainsize;
        else
            mdW{ilayer} = mommu * mdW{ilayer} + dW{ilayer}/trainsize;
            mdb{ilayer} = mommu * mdb{ilayer} + db{ilayer}/trainsize;
        end
